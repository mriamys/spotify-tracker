...
def get_latest_track_smart(sp, artist_id):
    ...
    try:
        ...
        sorted_releases = sorted(
            releases, key=lambda x: x.get("release_date", "0000-00-00"), reverse=True
        )
...
def run_daily_safe_scan():
    ...
    if last_run.date() == current_date and state["initial_scan_done"]:
        if state.get("monitoring_index", 0) == 0:
            print(
                f"[{datetime.now().strftime('%H:%M')}] ✋ Лимит на сегодня исчерпан (бот уже работал сегодня)."
            )
            return
    ...
    try:
        ...
        # ═══════════════════════════════════════════════════════
            ...
            while i < len(artists):
                artist = artists[i]
                artist_id = artist["id"]
                artist_last_date = state["artists_processed"].get(
                    artist_id, "2000-01-01"
                )
                ...
                try:
                    ...
                    for release in releases:
                        ...
                        if release_date > artist_last_date:
                            ...
                            if release_date > state["artists_processed"].get(
                                artist_id, "2000-01-01"
                            ):
